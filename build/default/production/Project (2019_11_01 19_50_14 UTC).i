...
typedef enum Richting{
    Links = 0,
    Rechts = 1
} richting;


volatile int Kanaal = 0;
volatile int Teller = 0;
volatile int RB5oud;

volatile char SchermUpdate = 0;
volatile unsigned int MaxSpanning = 0;
volatile unsigned int MinSpanning = 1023;
...
void UpdateKanalen(void)
{
    ...
    Kant = (Kant == (PORTBbits.RB5 == RB5oud));
    RB5oud = PORTBbits.RB5;
    ...
    Kanaal = (unsigned int)(((Teller) % (4*3))/3);
    PORTA = 0b1111 - (1 << Kanaal);
    SchermUpdate |= 2;
}
...
void Kalibreren(void)
{
    int i;
    int ADres;
    if(eeprom_read(0x2) >> 2 == 0b101010)
    {
        MinSpanning = eeprom_read(0x2) & 0b11;
        MinSpanning <<= 8;
        MinSpanning += eeprom_read(0x3);
    }
    else
    {
        PORTA = 0b1110;
        for(i =0 ; i <= 255 ;i++)
        {
            ...
            while( GO == 0b1 )continue;
            ADres = ADRESH << 8 | ADRESL;
            if(ADres < MinSpanning)
            {
                MinSpanning = ADres;
            }
        }

        eeprom_write(0x3, MinSpanning);
        eeprom_write(0x2, (MinSpanning >> 8) + (0b101010 << 2));
        _delay((unsigned long)((1000)*(4000000/4000.0)));
    }

    if(eeprom_read(0x0) >> 2 == 0b101010)
    {
        MaxSpanning = eeprom_read(0x0) & 0b11;
        MaxSpanning <<= 8;
        MaxSpanning += eeprom_read(0x1);
    }
    else
    {
        PORTA = 0b0111;
        for(i=0; i <= 255; i++)
        {
            ...
            while( GO == 0b1 )continue;
            ADres = ADRESH << 8 | ADRESL;
            if(ADres > MaxSpanning)
            {
                MaxSpanning = ADres;
            }
        }
            eeprom_write(0x1, MaxSpanning);
            eeprom_write(0x0, (MaxSpanning >> 8) + (0b101010 << 2));
    }
}
...
void UpdateVolume(void)
{
    _delay((unsigned long)((5)*(4000000/4000000.0)));
        ...
        if((int)((ADres + 50) * 4 / (MaxSpanning - MinSpanning)) == (int)((ADres - 50) * 4 / (MaxSpanning - MinSpanning)))
        {
            Volume = (int)(ADres * 4 / (MaxSpanning - MinSpanning));
